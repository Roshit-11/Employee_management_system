...
public class homepage1 extends javax.swing.JFrame {
   ...
   // Removed local attendanceList; use shared static store in AdminModel
    ...
    // Leave editing state (employee can edit only PENDING requests)
    private LeaveRequest editingLeave = null;
    private List<LeaveRequest> cachedMyLeaves = new ArrayList<>();
    ...
    public void refreshMyLeaveTable() {
        ...
        List<LeaveRequest> mine = AdminModel.getLeavesForUserFIFO(currentUsername);
        cachedMyLeaves = (mine == null) ? new ArrayList<>() : mine;

        for (LeaveRequest r : cachedMyLeaves) {
    ...
    // ---- Custom Leave Editing Behaviors ----
    private void initCustomBehaviors() {
        // Make employee tables non-editable from UI typing
        jTable2.setDefaultEditor(Object.class, null);
        jTable1.setDefaultEditor(Object.class, null);

        // When employee selects a leave row, allow edit ONLY if status is Pending
        jTable2.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) {
                onMyLeaveRowSelected();
            }
        });
    }

    private void onMyLeaveRowSelected() {
        int row = jTable2.getSelectedRow();
        if (row < 0 || row >= cachedMyLeaves.size()) {
            return;
        }

        LeaveRequest r = cachedMyLeaves.get(row);
        if (r == null) return;

        if (!"Pending".equalsIgnoreCase(r.getStatus())) {
            // Not editable once approved/rejected
            editingLeave = null;
        ...
        // Pending -> load into form for editing
        editingLeave = r;
        loadLeaveIntoForm(r);
        jButton5.setText("Update leave");
    }

    private void loadLeaveIntoForm(LeaveRequest r) {
        if (r == null) return;

        // From date
        jComboBox1.setSelectedItem(String.valueOf(r.getFromDate().getDayOfMonth()));
        jComboBox2.setSelectedItem(monthNumberToName(r.getFromDate().getMonthValue()));
        jComboBox3.setSelectedItem(String.valueOf(r.getFromDate().getYear()));

        // To date
        jComboBox4.setSelectedItem(String.valueOf(r.getToDate().getDayOfMonth()));
        jComboBox5.setSelectedItem(monthNumberToName(r.getToDate().getMonthValue()));
        jComboBox6.setSelectedItem(String.valueOf(r.getToDate().getYear()));

        // Leave type
        if ("Paid Leave".equalsIgnoreCase(r.getLeaveType())) {
            jRadioButton1.setSelected(true);
            jRadioButton2.setSelected(false);
        } else if ("Unpaid Leave".equalsIgnoreCase(r.getLeaveType())) {
    ...
    private void clearLeaveFormAndEditingState() {
        editingLeave = null;
        jButton5.setText("Apply for leave");
        jTextArea2.setText("");
    ...
    private void initComponents() {
        ...
        jCheckBox5.setText("    Complete monthly task");
        ...
        jCheckBox6.setText("    Attend team meeting at 3");
        ...
        jCheckBox7.setText("    Review project documentation");
        ...
        jCheckBox8.setText("    Submit timesheet");
        ...
        jButton9.setText("Mark all completed");
        ...
        leave_applicationLayout.setVerticalGroup(
            ...
            .addGroup(leave_applicationLayout.createSequentialGroup()
                .addContainerGap()
                .addComponent(jPanel25, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
        ...
        // If editing an existing PENDING request, remove old one and enqueue updated request
        if (editingLeave != null) {
            boolean removed = AdminModel.removeLeaveRequestFromQueue(editingLeave);
            if (!removed) {
                JOptionPane.showMessageDialog(this, "Could not update: original request not found.");
                return;
            }
        }
        ...
        JOptionPane.showMessageDialog(this,
                (editingLeave == null) ? "Leave request submitted (Pending)." : "Leave request updated (Pending)."
        );
